...
import idcby.cn.imagepicker.utils.Utils;
...
public class ImageSelectorActivity extends FragmentActivity implements ImageSelectorFragment.Callback {
    ...
    private void init() {
        ...
        if(!imageConfig.isReloadModel()){
            pathList.addAll(imageConfig.getPathList()) ;
        }
    ...
    private void SelectedFinish() {
        ...
        //data.putStringArrayListExtra(EXTRA_RESULT_UPLOAD, uploadPathList);
        setResult(RESULT_OK, data);
        LogUtils.showLog("我选择完照片了");
        //改变gridview的内容
        finish();
    ...
    private void crop(Context context , String imagePath, int aspectX, int aspectY, int outputX, int outputY) {
        File file;
        ...
        intent.setDataAndType(MyUtils.getFileUri(context ,new File(imagePath)), "image/*");
        ...
        intent.putExtra("return-data", false);//设置权限
    ...
    public void onSingleImageSelected(String path) {
        if (imageConfig.isCrop()) {
            crop(ImageSelectorActivity.this ,path, imageConfig.getAspectX(), imageConfig.getAspectY(), imageConfig.getOutputX(), imageConfig.getOutputY());
        } else {
            pathList.add(path);
            SelectedFinish();
        }
    ...
    public void onCameraShot(File imageFile) {
        if (imageFile != null) {
            if (imageConfig.isCrop()) {
                crop(ImageSelectorActivity.this ,imageFile.getAbsolutePath(), imageConfig.getAspectX(), imageConfig.getAspectY(), imageConfig.getOutputX(), imageConfig.getOutputY());
            } else {
