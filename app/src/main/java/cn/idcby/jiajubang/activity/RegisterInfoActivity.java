...
//        topLay.getLayoutParams().height = (int) (ResourceUtils.getScreenWidth(mContext) / 2.1F);
    ...
    public void dealOhterClick(View view) {
        ...
        if (R.id.acti_user_info_sex_tv == vId) { //性别
            showSexDialog();
        } else if (R.id.acti_user_info_head_iv == vId) {  //头像
            checkPhoto();
        } else if (R.id.acti_user_info_birthday_tv == vId) { //出生日期
...
//            SelectedProvinceActivity.launch(mActivity, REQUEST_CODE_AREA);
        } else if (vId == R.id.acti_user_info_category_tv) {//行业类别
...
//                    , mSelectedCategory, REQUEST_CODE_FOR_CATEGORY);
            ...
            submitModify();
        } else if (R.id.acti_user_info_work_name_tv == vId) { //职位选择
            showWordDialog();
        }
    }
        ...
        if (categoryDialog==null){
            ...
            categoryDialog.setDoubleSelectionInterface(new DoubleSelectionInterface() {
                ...
                public void onSelection(Object post) {
                    if (!(post instanceof UnusedCategory)){
                        return;
                    }
    ...
    private void showCityDialog() {
        if (cityDialog==null){
            ...
            cityDialog.setDoubleSelectionInterface(new DoubleSelectionInterface() {
                @Override
                public void onSelection(Object post) {
                    if (!(post instanceof Address)){
                        return;
                    }
                    Address address= (Address) post;
                    mAreaTv.setText(address.getAreaName());
