...
/**
 * 安装服务--我发布的、服务订单（提供的服务）
 * Created on 2018/5/19.
 */

public class MyCenterInstallActivity extends BaseActivity {
    ...
    public int getLayoutID() {
        return R.layout.activity_my_center_install;
    }
    ...
    public void initView() {
        ...
        mOrderLay = findViewById(R.id.acti_my_center_install_order_lay) ;
        mOrderIndicator = findViewById(R.id.acti_my_center_install_order_indicator) ;
        mOrderViewPager = findViewById(R.id.acti_my_center_install_order_vp) ;

        mSendLay = findViewById(R.id.acti_my_center_install_send_lay);
    ...
    private void initMySend(){
        ...
        manager.beginTransaction().add(mSendLay.getId()
                , MyServerFragment.newInstance(true)).commit() ;
    }
    ...
    private void initOrder(){
        ...
        fragmentList.add(FragmentServerOrder.getInstance(true ,0)) ;
        fragmentList.add(FragmentServerOrder.getInstance(true ,1)) ;
        fragmentList.add(FragmentServerOrder.getInstance(true ,2)) ;
        fragmentList.add(FragmentServerOrder.getInstance(true ,3)) ;
        fragmentList.add(FragmentServerOrder.getInstance(true ,4)) ;
        fragmentList.add(FragmentMyOrderAfterSale.getInstance(FragmentMyOrderAfterSale.ORDER_AFTER_SALE_FROM_MY
                ,FragmentMyOrderAfterSale.ORDER_ITEM_TYPE_INSTALL)) ;
