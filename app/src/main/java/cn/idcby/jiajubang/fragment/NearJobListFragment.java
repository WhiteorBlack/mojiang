...
public class NearJobListFragment extends BaseFragment {
...
//        mListType = getIntent().getIntExtra(SkipUtils.INTENT_JOB_TYPE , mListType) ;
        ...
        mLoadingLay =view. findViewById(R.id.acti_near_job_list_loading_lay) ;
        mNullTv = view.findViewById(R.id.acti_near_job_list_null_tv) ;
    ...
    protected int setSuccessViewId() {
        return R.layout.fragment_near_job_list;
    }
    ...
    private void intentToChooseResume(){
        if("".equals(SPUtils.newIntance(mContext).getToken())){
            SkipUtils.toLoginActivityForResult(NearJobListFragment.this , 1000) ;
        }else{
            Intent toCtIt = new Intent(mContext,ChooseResumeActivity.class) ;
            NearJobListFragment.this.startActivityForResult(toCtIt , 1001) ;
        }
    }
